...
function shouldBehaveLikeERC721(owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  context('with minted tokens', function () {
    beforeEach(async function () {
      const privateData1 = await fhenixjs.encrypt_uint128(1n);
      const privateData2 = await fhenixjs.encrypt_uint128(2n);
      await this.token.$_mint(owner, firstTokenId, privateData1);
      await this.token.$_mint(owner, secondTokenId, privateData2);
