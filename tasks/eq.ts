...
task("task:eq").setAction(async function (taskArguments: TaskArguments, hre) {
  const { ethers, deployments } = hre;

  const { deployer } = await hre.getNamedAccounts();
  const { deploy } = deployments;

  const deployResult = await deploy("Eq", {
    from: deployer,
    args: [],
    log: true,
  ...
  const contract = await ethers.getContractAt("Eq", deployResult.address);
  ...
  const testCases = [
    ...
    {
      function: "eq(euint8,euint16)",
      cases,
    },
    {
      function: "eq(euint8,euint32)",
      cases,
    },
    {
      function: "eq(euint8,uint8)",
      cases,
    },
    {
      function: "eq(uint8,euint8)",
      cases,
    },
    {
      function: "eq(euint16,euint8)",
      cases,
    },
    {
    ...
    {
      function: "eq(euint16,euint32)",
      cases,
    },
    {
      function: "eq(euint16,uint16)",
      cases,
    },
    {
      function: "eq(uint16,euint16)",
      cases,
    },
    {
      function: "eq(euint32,euint8)",
      cases,
    },
    {
      function: "eq(euint32,euint16)",
      cases,
    },
    {
    ...
    {
      function: "eq(euint32,uint32)",
      cases,
    },
    {
      function: "eq(uint32,euint32)",
      cases,
    },
  ];
  ...
  for (const test of testCases) {
    for (const testCase of test.cases) {
      try {
        const encryptedOutput = await contract.connect(signer).eq(testCase.a, testCase.b, test.function, publicKey);
