...
task("task:ne").setAction(async function (taskArguments: TaskArguments, hre) {
  const { ethers, deployments } = hre;

  const { deployer } = await hre.getNamedAccounts();
  const { deploy } = deployments;

  const deployResult = await deploy("Ne", {
    from: deployer,
    args: [],
    log: true,
  ...
  const contract = await ethers.getContractAt("Ne", deployResult.address);
  ...
  const testCases = [
    ...
    {
      function: "ne(euint8,euint16)",
      cases,
    },
    {
      function: "ne(euint8,euint32)",
      cases,
    },
    {
      function: "ne(euint8,uint8)",
      cases,
    },
    {
      function: "ne(uint8,euint8)",
      cases,
    },
    {
      function: "ne(euint16,euint8)",
      cases,
    },
    {
    ...
    {
      function: "ne(euint16,euint32)",
      cases,
    },
    {
      function: "ne(euint16,uint16)",
      cases,
    },
    {
      function: "ne(uint16,euint16)",
      cases,
    },
    {
      function: "ne(euint32,euint8)",
      cases,
    },
    {
      function: "ne(euint32,euint16)",
      cases,
    },
    {
    ...
    {
      function: "ne(euint32,uint32)",
      cases,
    },
    {
      function: "ne(uint32,euint32)",
      cases,
    },
  ];
  ...
  for (const test of testCases) {
    for (const testCase of test.cases) {
      try {
        const encryptedOutput = await contract.connect(signer).ne(testCase.a, testCase.b, test.function, publicKey);
